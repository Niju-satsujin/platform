...
You do not know if your code is fast or slow until you measure it. "It feels fast" is not data. You need a number: how many log entries per second can the logger write?
...
This benchmark becomes your **baseline**. Every future change must not make these numbers worse. If you refactor the logger in week 3 and ops/sec drops by 50%, you know you broke something.

## Task

1. Create `benchmark.cpp` that:
   - Creates a Logger with `FsyncPolicy::NONE`
   ...
   - Creates a Logger with `FsyncPolicy::EVERY_WRITE`
   - Writes 1,000 entries (fewer because fsync is slow)
   - Prints: `"fsync: <N> ops/sec"`
2. Add it to CMakeLists.txt as `add_executable(logger_benchmark benchmark.cpp)`
