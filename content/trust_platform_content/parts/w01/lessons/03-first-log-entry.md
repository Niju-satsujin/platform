...
proof:
  type: paste
  instructions: "Paste the content of your log file showing at least 3 entries with timestamps, levels, components, and messages separated by tabs."
  regex_patterns:
    - "\\d{13}"
    - "\\t"
    - "INFO|WARN|ERROR"
...
A useful log entry has four pieces of information:
...
The format is tab-separated: `timestamp\tlevel\tcomponent\tmessage\n`
...
4. Write a test in `main.cpp` using `assert()`:
   ...
   - Assert each line has exactly 4 tab-separated fields
...
Expected — 3 lines, each with 4 tab-separated fields:
```
...
(Your timestamps will be different — that is correct.)

## Done When

The assert passes: each line has exactly 4 tab-separated fields and the timestamp is a 13-digit number.
