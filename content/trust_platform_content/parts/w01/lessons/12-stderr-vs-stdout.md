...
proof:
  type: paste
  instructions: "Paste output of: (1) 'logger read > /dev/null' showing only stderr content, (2) 'logger read 2>/dev/null' showing only stdout content. Prove data and errors go to separate streams."
  regex_patterns:
    - "/dev/null"
---
...
- Debug/status messages (like "Logger opened") go to `std::cerr` (stderr)
...
5. These should be on stderr: error messages, warnings about skipped lines, "Logger opened/closed" messages
...
- `--help` output should go to stdout (so `logger --help | less` works)

## Verify

```bash
# Should show only errors/warnings (stderr), no data
./logger read > /dev/null

# Should show only data (stdout), no errors
./logger read 2>/dev/null

# Counting entries should be accurate
./logger read 2>/dev/null | wc -l
```
...
`./logger read 2>/dev/null | wc -l` returns the exact number of valid log entries with zero error messages mixed in.
